...
// the path, from the document root of the server, to the directory that contains
// all study directories.
$studies_directory = '.';
...
preg_match("`" . $studies_directory . "/([a-zA-Z0-9\._-]+)`", $_SERVER["HTTP_REFERER"], $match);
$study_directory = explode("/", $match[0]);
$study_directory = $study_directory[1];
$study_include_path = $_SERVER["DOCUMENT_ROOT"] . "/" . $studies_directory . "/" . $study_directory; 
